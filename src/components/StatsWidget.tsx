...
function MaterialIcon({ material }: { material: MaterialKey }) {
  switch (material) {
    case 'wood':
      return (
        <svg viewBox="0 0 24 24" className="stats-widget__material-icon">
...
export default function StatsWidget() {
  ...
  return (
    <div className="stats-widget">
      {/* Header */}
      <div className="stats-widget__section-header">Boat Statistics</div>

      {/* HP & Mass */}
      <div className="stats-widget__row">
        <span className="stats-widget__row-label">HP</span>
        <span className="stats-widget__row-value">{totalHp.toLocaleString()}</span>
      </div>
      <div className="stats-widget__row">
        <span className="stats-widget__row-label">Mass</span>
        <span className="stats-widget__row-value">{totalMass.toLocaleString()}</span>
      </div>

      {/* Speed */}
      <div className="stats-widget__section-header stats-widget__speed-header">
        <span>Speed</span>
        <svg viewBox="0 0 16 16" className="stats-widget__info-icon">
          <circle cx="8" cy="8" r="7" fill="none" stroke="currentColor" strokeWidth="1.5" />
          <text x="8" y="12" textAnchor="middle" fill="currentColor" fontSize="10" fontWeight="bold">i</text>
        </svg>
        <div className="stats-widget__tooltip">
          Assumes no drag penalty. Multiple flat edges facing the direction of travel will reduce max speed below 54 km/h.
        </div>
      </div>
      <div className="stats-widget__row">
        <span className="stats-widget__row-label">Sails</span>
        <span className="stats-widget__row-value">
          {speedInfo.canAchieveWithSails ? speedInfo.sailsNeeded : 10}
        </span>
      </div>
      {!speedInfo.canAchieveWithSails && speedInfo.enginesNeeded !== null && (
        <div className="stats-widget__row">
          <span className="stats-widget__row-label">+ Engines</span>
          <span className="stats-widget__row-value">{speedInfo.enginesNeeded}</span>
        </div>
      )}
      <div className="stats-widget__speed-status">
        ...
        {speedInfo.canAchieveMaxSpeed ? (
          ...
          <span className="stats-widget__speed-fail">&#x2718;</span>
        )}
      </div>

      {/* Cost to Sink */}
      <div className="stats-widget__section-header">Cost to Sink</div>
      {raidCosts.map((entry) => (
        <div key={entry.key} className="stats-widget__raid-row">
          <span className="stats-widget__raid-count">
            {entry.count.toLocaleString()}x {entry.label}
          </span>
          <span className="stats-widget__raid-sulfur">{entry.sulfur.toLocaleString()} S</span>
        </div>
      ))}
      ...
      {activeMaterials.length > 0 && (
        <>
          <div className="stats-widget__section-header">Materials</div>
          {activeMaterials.map((mat) => (
            <div key={mat} className="stats-widget__material">
              <MaterialIcon material={mat} />
              <span className="stats-widget__material-amount">{(totals[mat] ?? 0).toLocaleString()}</span>
              <span className="stats-widget__material-label">{MATERIAL_SHORT_LABELS[mat]}</span>
            </div>
          ))}
        </>
      )}
